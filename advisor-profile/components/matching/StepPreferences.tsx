...
type Props = {
  ...
  onNext: (payload: { budgetLakh: number; travelStyle: string }) => void
  onBack?: () => void
}
...
export default function StepPreferences({
  initialBudgetLakh = 15,
  initialTravelStyle = null,
  onNext,
  onBack,
}: Props) {
  ...
  const canContinue = Boolean(travelStyle)
  ...
  return (
    <div className="mx-auto flex w-full max-w-[38rem] flex-col px-4 sm:px-6">
      ...
      >
        Set your budget and who is travelling — then we match.
      </p>
      ...
      >
        ...
        <div className="relative mb-4 h-2.5 rounded-full bg-[var(--teal-light)]">
          ...
          <input
            ...
            aria-describedby="budget-hint budget-display"
      ...
      <fieldset className="mb-6 border-0 p-0">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3" role="radiogroup" aria-labelledby="style-label">
        {STYLES.map((s) => {
          const sel = travelStyle === s.id
          return (
            <button
              key={s.id}
              id={`travel-style-${s.id.toLowerCase()}`}
              type="button"
              name="travelStyle"
              value={s.id}
              aria-pressed={sel}
      ...
      <div className="text-center">
        {canContinue && (
          <button
            ...
            type="button"
            onClick={() => travelStyle && onNext({ budgetLakh, travelStyle })}
